...
export default {
  ...
  getMsgForEmployee: 'select message.id as id, msg_title, msg_content, project_name, creation_time, publisher from message join project on message.project_name = project.name where plant = ? and del_mark = 0 order by creation_time desc',
  getMsgForResearcher:'select distinct M.id , M.msg_title, M.msg_content, project_name, creation_time, publisher ' +
